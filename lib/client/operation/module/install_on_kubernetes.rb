...
module DTK::Client
  class Operation::Module
    class InstallOnKubernetes < self
      ...
      def initialize(file_obj, module_ref, has_remote_repo, install_from)
        ...
        @print_helper     = Install::PrintHelper.new(:module_ref => @base_module_ref, :source => install_from || :local) # ? :remote : :local)
      end
      private :initialize
      ...
      def install(opts = {})
        ...
        self.class.handle_error base_path do
          Install::CommonModule.install_on_kubernetes(@base_module_ref, @file_obj, :has_remote_repo => @has_remote_repo)
        end
